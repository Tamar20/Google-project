...
def search_best_completions(prefix: str, trie: Trie):
    ...
    for comp_dict in complete_list:
        ...
        auto_complete_list.append(auto_comp_obj)

    return sort_res_by_score(auto_complete_list)[:5]


def sort_res_by_score(auto_complete_list: list):
...
def search_with_mistake(sentence: str, trie: Trie):
    return add_character(sentence, trie)
    return delete_character(sentence, trie)
    return replace_character(sentence, trie)
